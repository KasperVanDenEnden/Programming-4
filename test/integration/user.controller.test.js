...
describe("UC-201 Manage users /api/user ", () => {
    ...
    it("TC-201-2 When the email is not equal to the regex, a valid error should be returned", (done) => {
      chai
        ...
        .end((err, res) => {
          ...
          let { status, result } = res.body;
          status.should.equals(400);
          result.should.be.a("string").that.equals("No valid email format");
          done();
        });
    });
  });
  ...
  describe("TC-201-4 User already exist", () => {
    it("Creating the user for test, a valid respons should be given", (done) => {
        chai
          .request(server)
          .post("/api/user")
          .send({
          ...
          .end((err, res) => {
            ...
            result.should.be.a("string").that.equals("User is added!");
            done();
          });
      });
      ...
      it("TC-201-4 When a user already exists, a valid error should be returned", (done) => {
        ...
        chai
          ...
          .end((err, res) => {
            ...
            result.should.be.a("string").that.equals("Email is not unique!");
            done();
          });
      });
    });
...
describe("UC-202 Request list of users /api/user", () => {
  ...
  describe("TC-202-2 Show two users", () => {
    beforeEach((done) => {
      users = [];
       const objOne = {id:1, firstName: 'Kasper', lastName: 'van den Enden', isActive: true, emailAdress: 'k.test@test.nl', phoneNumber: '06 36391089', city: 'Breda',postcode: "4824 LT"}
       const objTwo = {id:2, firstName: 'Steven', lastName: 'van den Enden', isActive: true, emailAdress: 's.test@test.nl', phoneNumber: '06 12345678', city: 'Breda', postcode: "4824 LT" }
  ...
  describe("TC-205-3 No valid phonenumber", () => {
    it('TC-205-3 When no valid phonenumber is given, an error should be returned', (done) => {
      ...
      .end((err, res) => {
        ...
        let { status, result } = res.body;
        status.should.equals(400);
        result.should.be.a("string").that.equals("No valid phonenumber format");
        done();
      });
    })
  });
  ...
  describe("TC-205-4 User does not exist", () => {
    it('TC-205-4 When the user does not exist, an eroor should be returned', (done) => {
      ...
      .end((err, res) => {
        ...
        let { status, result } = res.body;
        status.should.equals(400);
        result.should.be.a("string").that.equals("User with ID 57 not found");
        done();
      });
    })
  });
  ...
  describe("TC-206-1 User does not exist", () => {
    it('TC-206-1 When the user does not exist, a error should be given', (done) => {
      ...
      .end((err,res) => {
        ...
        let { status, result } = res.body;
        status.should.equals(400);
        result.should.be.a("string").that.equals("No user with id 57 was found");
        done();
      })
    })
